...
assert_grep '"success": true' "$DRY_OUT" "reclassify-01 dry-run succeeds"
assert_grep '"action": "dry_run"' "$DRY_OUT" "reclassify-02 dry-run is the default"
assert_grep '"slug": "andrej-karpathy"' "$DRY_OUT" "reclassify-03 person entity listed as candidate"
if python3 -c "
import json,sys
d = json.load(open('$DRY_OUT'))
sys.exit(0 if all(c['slug'] != 'fraunhofer-institut' for c in d['data']['candidates']) else 1)
"; then
  green "PASS: reclassify-04 org entity is NOT a candidate (org marker)"
else
  red "FAIL: reclassify-04 org entity surfaced as a person candidate"
  errors=$((errors + 1))
fi
assert_grep '"slug": "j-robert-oppenheimer"' "$DRY_OUT" \
  "reclassify-05 dotted-initials person listed as candidate (not an acronym false-negative)"
if [ -f "$WIKI/wiki/entities/andrej-karpathy.md" ] && [ ! -e "$WIKI/wiki/people/andrej-karpathy.md" ]; then
  green "PASS: reclassify-06 dry-run moved nothing"
else
  red "FAIL: reclassify-06 dry-run moved files"
  errors=$((errors + 1))
fi
...
assert_grep '"success": false' "$NOSEL_OUT" "reclassify-07 --apply without selector refused"
assert_grep 'requires an explicit selector' "$NOSEL_OUT" "reclassify-08 refusal names the selector contract"
...
assert_grep '"success": true' "$APPLY_OUT" "reclassify-09 apply succeeds"
assert_grep '"action": "reclassified"' "$APPLY_OUT" "reclassify-10 apply reports reclassified"
if [ -f "$WIKI/wiki/people/andrej-karpathy.md" ] && [ ! -e "$WIKI/wiki/entities/andrej-karpathy.md" ]; then
  green "PASS: reclassify-11 page moved into wiki/people/"
else
  red "FAIL: reclassify-11 page not moved"
  errors=$((errors + 1))
fi
assert_grep '^type: person$' "$WIKI/wiki/people/andrej-karpathy.md" "reclassify-12 frontmatter retyped to person"
assert_grep 'A named human filed as an entity' "$WIKI/wiki/people/andrej-karpathy.md" \
  "reclassify-13 body preserved byte-for-byte"
if python3 -c "
import sys
t = open('$WIKI/wiki/people/andrej-karpathy.md').read()
sys.exit(0 if 'type: person\n\ncreated:' in t else 1)
"; then
  green "PASS: reclassify-14 blank line after the retyped frontmatter line preserved (regex does not eat newlines)"
else
  red "FAIL: reclassify-14 blank line after type: line was consumed by the retype"
  errors=$((errors + 1))
fi
assert_grep '^type: entity$' "$WIKI/wiki/entities/fraunhofer-institut.md" "reclassify-15 org page untouched"
if [ -f "$WIKI/wiki/people/index.md" ] && [ -f "$WIKI/wiki/entities/index.md" ]; then
  green "PASS: reclassify-16 entities + people sub-indexes rendered"
else
  red "FAIL: reclassify-16 sub-indexes not rendered"
  errors=$((errors + 1))
fi
assert_grep 'andrej-karpathy' "$WIKI/wiki/people/index.md" "reclassify-17 people sub-index carries the moved page"
assert_grep 'reclassify' "$WIKI/wiki/meta/log.md" "reclassify-18 reclassification logged"
assert_grep '"entries_count": 3' "$WIKI/.cogni-wiki/config.json" "reclassify-19 entries_count unchanged"
...
assert_grep '"success": true' "$NOOP_OUT" "reclassify-20 second run succeeds"
assert_grep '"action": "noop"' "$NOOP_OUT" "reclassify-21 second run is a noop"
assert_grep '"reason": "already_reclassified"' "$NOOP_OUT" "reclassify-22 noop reason is already_reclassified"
...
assert_grep '"success": false' "$CLOB_OUT" "reclassify-23 existing target refused"
assert_grep '"reason": "target_exists"' "$CLOB_OUT" "reclassify-24 refusal reason is target_exists"
assert_grep 'Existing person page.' "$WIKI2/wiki/people/andrej-karpathy.md" "reclassify-25 existing target not clobbered"
assert_grep '^type: entity$' "$WIKI2/wiki/entities/andrej-karpathy.md" "reclassify-26 source left intact on refusal"
...
assert_grep '"success": false' "$LEGACY_OUT" "reclassify-27 pre-0.0.8 base refused"
assert_grep 'knowledge-index --migrate' "$LEGACY_OUT" "reclassify-28 refusal points at the migration path"
