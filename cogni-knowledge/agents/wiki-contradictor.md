...
description: Phase-7 zero-network contradiction scorer for the inverted pipeline. Reads the just-deposited <wiki>/syntheses/<slug>.md and each cited page's claim frontmatter, walks the synthesis body sentence-by-sentence against every claim, and emits <project>/.metadata/contradictor-vN.json (schema 0.1.0) with findings carrying kind ∈ {contradiction, unknown} and severity ∈ {high, medium, low}. Pure observability — no auto-resolution, no rollback, no behaviour change downstream.
model: sonnet
color: orange
