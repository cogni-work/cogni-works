...
- `cogni-workspace` (claims capability) — **required only for `knowledge-refresh --resweep`**, the opt-in live-source claim re-verification. This is the same plugin as the market-registry entry above, not a second dependency. That flag is orthogonal and never auto-runs, so it is the one path that leaves the zero-network default: it dispatches `cogni-workspace:claims submit` / `verify` to re-check the bound wiki's cited claims against their live source URLs. Nothing else reaches it — a push-mode refresh never probes it, and `knowledge-compose`, `knowledge-verify` and `knowledge-finalize` are zero-network by design and verify citations against claims extracted at ingest time instead. When it is absent, `--resweep` **aborts** with an install pointer rather than degrading silently; drop `--resweep` and every other run is unaffected.
