...
- **Word-count enforcement chain**: writer commits to a per-section budget in `.metadata/writer-outline-v{N}.json` → Phase 4.5 measures the written file via `wc -w` (self-reported `words` is ignored for the gate) → re-dispatches once with `TARGET_MIN_WORDS` + `EXPANSION_NOTES` if below `floor × 0.9` → reviewer's Word Count Gate applies a stepped completeness cap (0.30 / 0.45 / 0.60 by delivered-to-minimum ratio) and emits a `Word deficit:` issue → revisor lifts its +20% cap in expansion mode → Phase 5 runs a second structural review iteration only when the verdict has a `Word deficit` issue → Phase 6 surfaces the delivered-vs-target comparison in the user summary. The `allow_short: true` flag short-circuits the re-dispatch and the second review iteration
