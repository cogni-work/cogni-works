...
if [ ! -d "$WSD" ]; then
  red "FAIL: repair-01 vendored cogni-wiki wiki-ingest scripts not found at $WSD"
  exit 1
fi
...
assert_grep '"success": true' "$DRY" "repair-02-ac1-1 dry-run succeeds"
assert_grep '"action": "dry_run"' "$DRY" "repair-03-ac1-2 dry-run is the default (no --apply)"
assert_grep '"reason": "repair_pending"' "$DRY" "repair-04-ac1-3 dry-run reason is repair_pending"
assert_grep 'ROOT-LINKS' "$DRY" "repair-05-ac1-4 dry-run names the drifted ROOT-LINKS region"
assert_grep 'root-index-proposed.md' "$DRY" "repair-06-ac1-5 dry-run stages the proposed root MAP"
if cmp -s "$WIKI/wiki/index.md" "$WORK/degraded-index.before"; then
  green "PASS: repair-07-ac1-6 dry-run leaves the live index.md byte-identical"
else
  red "FAIL: repair-07-ac1-6 dry-run mutated the live index.md"
  errors=$((errors + 1))
fi
...
assert_grep '"success": true' "$APPLY" "repair-08-ac1-7 --apply succeeds"
assert_grep '"action": "repaired"' "$APPLY" "repair-09-ac1-8 --apply action is repaired"
assert_not_grep "$ROOT_LINKS_EMPTY" "$WIKI/wiki/index.md" \
  "repair-10-ac1-9 --apply replaced the empty ROOT-LINKS sentinel in the live index.md"
assert_grep 'Sources (1)' "$WIKI/wiki/index.md" \
  "repair-11-ac1-10 --apply populated the ROOT-LINKS span with the theme-scoped count-link"
assert_grep 'curated-layout repair' "$WIKI/wiki/meta/log.md" \
  "repair-12-ac1-11 --apply appended a repair log line"
...
assert_grep '"success": true' "$HOUT" "repair-13-ac2-1 health succeeds on the repaired base"
assert_not_grep 'structural_drift' "$HOUT" \
  "repair-14-ac2-2 repaired base fires no structural_drift"
assert_grep '"errors": 0' "$HOUT" "repair-15-ac2-3 repaired base has zero errors"
...
assert_grep '"action": "noop"' "$NOOP" "repair-16-ac3-1 clean base reaches action:noop"
assert_grep '"reason": "no_drift_detected"' "$NOOP" "repair-17-ac3-2 clean base reason is no_drift_detected"

NOOP2="$WORK/noop2.json"
python3 "$SCRIPT" --wiki-root "$CLEAN" --wiki-scripts-dir "$WSD" --repair --apply > "$NOOP2"
assert_grep '"action": "noop"' "$NOOP2" "repair-18-ac3-3 second --apply on a clean base is a clean no-op"
...
assert_grep '"action": "repaired"' "$LAGOUT" "repair-19-lag-1 --apply repairs the schema lag"
assert_grep '"schema_after": "0.0.9"' "$LAGOUT" "repair-20-lag-2 --apply reports schema_after 0.0.9"
assert_grep '"schema_version": "0.0.9"' "$LAG/.cogni-wiki/config.json" \
  "repair-21-lag-3 --apply bumped the on-disk schema_version to 0.0.9"
...
assert_grep '"success": false' "$GUARD" "repair-22-guard-1 pre-0.0.8 base is refused"
assert_grep 'migrate' "$GUARD" "repair-23-guard-2 refusal points at the full migration (--migrate)"
