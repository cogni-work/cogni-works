...
- `narrative` transforms structured input into an executive narrative using one of 15 story arcs and, with `--format`, condenses an existing narrative into executive briefs, talking points or one-pagers. Agents: `narrative-writer`, `narrative-adapter`. The skill is a control plane over one-responsibility files: `SKILL.md` orchestrates six phases and four drafting passes; `references/story-arc/arc-registry.md` chooses (one declarative block per arc plus the detection algorithm and the shortlist confirmation); each arc is **one contract file**, `references/story-arc/{arc}/arc-definition.md`, carrying `contract: 2` frontmatter and a fixed heading set — Intent, Selection, Headings, Composition, Elements, Validation, See Also; `references/narrative-techniques/techniques-overview.md` strengthens; `references/language/{shared,en,de}.md` express, loaded at Pass 3 only; `references/validation.md` is the single home of every universal gate, its deterministic half run by `scripts/validate-narrative.py`. The earlier three-layer arc structure — an arc definition, a per-arc workflow file and four never-loaded per-element files — and the separate localized-header catalogue are gone; `tests/test-arc-contract-shape.sh` keeps every arc on the one-file shape, `tests/test-arc-taxonomy-sync.sh` case H1 derives the render side's short element names from the contracts' headings, `tests/test-narrative-validate.sh` proves each deterministic gate goes red on a narrative that breaks precisely its rule and green on three conforming fixtures, and `tests/test-narrative-bridge-citations.sh` pins the citation bridge on both input shapes
- `text-to-narrative` is the narrative skill's successor and, for now, its sibling: it runs the same Phases 0-6 from its **own bundled copy** of every asset — the registry, the fifteen arc contracts, the techniques overview, the language files, the execution brief, the validation rules and both scripts — and adds Phase 7, which cuts the finished narrative into one `design-brief.md` for Claude Design (slides, document, infographic or web). The bundled tree is **flat** (`references/arc-{arc}.md`, `references/language-{shared,en,de}.md`, `references/techniques-overview.md`): a verbatim copy of the nested tree would have added one `reference-depth` finding per directory to the shrink-only skill-spec baseline, and the baseline's own comment names flattening as the restructure. `scripts/flatten-narrative-assets.py` is the single statement of every delta between origin and copy (file mapping, relative-path rewrites, two prose edits); run without `--write` it is the identity check `tests/test-text-to-narrative-brief.sh` case `ttn-20` calls, so the copies cannot drift while both skills exist — add an arc upstream and re-derive, never hand-edit a vendored file. `references/density-ceilings.md` is the one home of every text-length ceiling the brief applies, one table per target, read at run time by `scripts/check-design-brief.py`; `tests/test-brief-density-sync.sh` is the transfer proof that each number equals the `story-to-*` or validator home it was carried from, and loses one arm per retired home. `references/design-brief-template.md` owns the brief grammar; the five green fixtures under `tests/fixtures/design-brief/` are its worked examples. **Retirement path, stated mechanically so it cannot go stale:** retiring `narrative` means deleting its 18 skill-spec baseline entries and then visiting **every tracked file outside `skills/narrative/` and `skills/text-to-narrative/` that names `skills/narrative/`** — found by `git grep -l 'skills/narrative/'` at retirement time, never from a list written here. Today that set includes the copywriter's six runtime-read surfaces pinned by `tests/test-arc-reference-sync.sh`, two of `tests/test-brief-density-sync.sh`'s homes (`VALIDATOR` and `ARC_CONTRACT`, whose absence fails `bds-01` and exits the suite), `tests/test-arc-taxonomy-sync.sh`, `tests/test-arc-contract-shape.sh`, the `test-narrative-*.sh` suites, `commands/narrative-adapt.md`, `libraries/arc-taxonomy.md`, the `narrative-writer` / `narrative-adapter` agents and `narrative-publish` — each repointed at the flat copies or retired with the skill — plus the identity and phase-parity cases `ttn-19` / `ttn-20`, which are dropped. Retiring a `story-to-*` skill is the same grep on `skills/story-to-<name>/`, and for this skill it means deleting that home's arm of the density-sync suite. `commands/text-to-narrative.md` registers `/text-to-narrative`
...
The copywriter's test payload came across with the skill and now lives outside it, at `tests/fixtures/copywriter/`: `readability-rule/run.sh` is the standing harness for the translation readability rule, `test-docs/arc-narrative.md` is also an input to cogni-visual's story-to-web eval, and `readability.yml` is the readability band the `narrative` skill's Pass 4 measures against. They are fixtures and contracts consumed from outside the skill, not eval run artifacts, which is why they sit under `tests/` rather than in the skill directory — `scripts/check-skill-spec.py` grades a skill directory on what a model would have to page through to use the skill. The dependency the story-to-web eval has on `arc-narrative.md` is recorded in `references/copywriter-changelog.md`, the skill's former root-level CHANGELOG, moved out for the same reason. The 27 eval *run* artifacts under the former `copywriter-workspace/` were pruned; the fixture files were kept, and the payload now under `tests/fixtures/copywriter/` is 19 files (`readability-rule/` 8, `test-docs/` 10, and `readability.yml`).
