...
> **Strategy Advisor voice** — this plugin ships one advisory output style: **Strategy Advisor** (answer-first, MECE options), language-neutral, so it renders in the resolved interaction language. Enable it from the `/config` output-style picker; it's opt-in and fixed at session start, so set it before scoping begins.
