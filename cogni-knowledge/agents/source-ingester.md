...
5. **Capture the dispatch start time** for the Phase-4 `duration_ms` field. Record a monotonic millisecond timestamp at the very start of work, before the Phase-1 cache read and the Phase-2 `claim-extractor` dispatch, so the measured wall clock spans this agent's full lifetime (including the sub-agent):
   ...
   Hold `START_MS` for Phase 4. **Each Bash tool call runs in a fresh shell**, so it does *not* survive as a shell variable — record the literal integer and re-type it (or set it inline) in the Phase-4 duration compute; never persist it to a fixed-named file in the shared session scratchpad, where a concurrent sibling would clobber it. This runs even on the skip paths, so a skip envelope can still report its `duration_ms`.

6. **Create the per-dispatch scratch root.** The session scratchpad is **shared across every parallel sibling `source-ingester` dispatch in the same ingest wave**, so an artifact written there under a fixed, generic name can be clobbered mid-dispatch — this dispatch would then read *another* source's body and mis-attribute its claims/provenance. The scratch dir must therefore be **unique per dispatch** AND **recomputable in a later, fresh shell** (each Bash tool call runs in a fresh shell, so a `WORK_DIR` shell variable does not survive into the Phase-1 / Phase-3 / cleanup calls). A random `mktemp -d` satisfies the first but not the second, and bridging that value through a fixed-named file reintroduces the clobber hazard. Derive the path **deterministically from the already-validated, per-dispatch-unique `SLUG`** instead:
   ...
   `SLUG` was validated at step 2 (`^[a-z0-9][a-z0-9-]{0,79}$` — lowercase, dashes, no `/` or shell metacharacters) and the orchestrator dedupes slugs pre-fan-out, so distinct sources get distinct `WORK_DIR`s and a sibling can never collide. `WORK_DIR` is this dispatch's load-bearing isolation boundary: the Phase-1 body file and the Phase-3 composed page — the only two scratch artifacts this agent writes — both live under it. Because the path is a pure function of `SLUG`, **re-type this exact literal expression at the top of every later Bash call that needs it** (Phases 1, 3, and the cleanup); never persist the value to a bridge file. The leading `rm -rf` also makes a same-`SLUG` retry (e.g. after an `integrity_mismatch` skip) idempotent, clearing any `body.txt` / `page.md` left by a crashed dispatch, where `mktemp -d` would leak a fresh directory each retry. `mktemp` is also not portably deterministic (BSD appends its own random suffix, GNU requires a literal `XXXXXX`), so its path cannot be recomputed in a later call.
...
Otherwise, take `data.entry.body` as the source body. Write it to the per-dispatch body file `$WORK_DIR/body.txt` (the Phase-0 scratch root — re-type the `WORK_DIR="${TMPDIR:-/tmp}/cogni-knowledge-ingest-${SLUG}"` literal in this call, per Phase 0 step 6) for the `claim-extractor` dispatch — pass a path, not a string, so the extractor stays Read-only.
...
Recompute `excerpt_position` — never trust the extractor's hand-counted value. **Decide present-vs-absent with the normalized matcher, not a raw substring test**: run a `python3` one-liner that puts `${CLAUDE_PLUGIN_ROOT}/scripts` on `sys.path`, does `from _knowledge_lib import excerpt_present`, and evaluates `excerpt_present(excerpt_quote, body)`. The matcher tolerates the mechanical PDF-vs-HTML drift (ligature codepoints, smart quotes, column-wrap intra-word newlines) a raw `in` / `find()` test penalizes — without it, PDF sources lose claims their clean HTML twins keep and the page is quarantined at the Step 3.5 integrity sweep. Then, per emitted claim:
...
author: "<AUTHOR>"                # only when a byline is confidently detected
published_date: "<PUBLISHED_DATE>"  # only when a publication date is confidently detected
...
- **Emit `author:` only when a byline is confidently detected in the fetched source** (drop the line entirely otherwise — the key is OPTIONAL; a consumer falls back to `publisher:`, so a dropped line degrades rather than breaks). Never infer or guess a byline. When present, quote it with `json.dumps`. Derived from the fetched body, not a dispatch parameter.
- **Emit `published_date:` only when a publication date is confidently detected in the fetched source** (drop the line entirely otherwise — OPTIONAL for the same reason; a consumer falls back to the year component of `fetched_at:`). Never infer or guess a date. When present, quote it with `json.dumps`. Derived from the fetched body, not a dispatch parameter.
...
`content_hash` is the **provenance hash of the fetched source body** (from `entry.content_hash`), not a hash of this markdown file. The on-disk page is the fetched body plus the injected `# <title>` H1 and the `Type: …` line, and downstream backlink maintenance (`knowledge-ingest`'s `backlink_audit.py --apply-plan`, `knowledge-finalize`'s `lint --fix=reverse_link_missing`) may append a `## See also` trailer. So a future integrity check must compare `content_hash` against the **fetched body in the cache**, never against `hash(<on-disk page body>)` — the latter diverges by design once backlinks are written, and `excerpt_position` offsets (anchored to the verbatim body that precedes any appended trailer) stay valid.

First write the composed page markdown to the per-dispatch scratch file `$WORK_DIR/page.md` (the Phase-0 scratch root — re-type the `WORK_DIR="${TMPDIR:-/tmp}/cogni-knowledge-ingest-${SLUG}"` literal in this call, per Phase 0 step 6). Then write atomically via `_knowledge_lib.atomic_write_text` against `<WIKI_ROOT>/wiki/<page-type-dir>/<slug>.md` — with the default `PAGE_TYPE=source` this is `<WIKI_ROOT>/wiki/sources/<slug>.md`; for `PAGE_TYPE=interview` it is `<WIKI_ROOT>/wiki/interviews/<slug>.md` (the `<page-type-dir>` resolved in Phase 0). Pass paths via env vars so apostrophes / spaces in WIKI_ROOT or tmp paths cannot break the Python literal.

**Pre-write integrity assertion (fail-fast).** `SLUG` and `URL` are the authoritative identity of the page you are writing, and `entry.content_hash` (from `fetch-cache.py fetch` in Phase 1) the authoritative provenance hash of its body. Because many ingesters fan out in one wave, a page can be composed from a sibling source's body/frontmatter by mistake; this guard refuses to let such a cross-written page reach disk. Add `SLUG`, `URL`, and `CONTENT_HASH` (the Phase-1 `entry.content_hash`) as env vars and, before calling `atomic_write_text`, parse the composed page's frontmatter and assert the page's `id:` equals `SLUG`, its first `sources:` URL normalizes to `URL`, the target path stem equals `SLUG`, and — when the page emitted a `content_hash:` line — that it equals `CONTENT_HASH`. The content_hash leg catches the narrower variant where you kept your own `id:`/`sources:` but the body (and its `content_hash:` line) bled from a sibling: page frontmatter is freeform output that can diverge under cross-talk, while `CONTENT_HASH` is the deterministic Phase-1 cache value for the dispatched URL, so the comparison is not tautological. On any mismatch, write **nothing** and `sys.exit(3)`:
...
A skip path still reports `duration_ms` (computed the same way) so a wave where every dispatch skipped still records a meaningful `max_agent_duration_ms` for the phase.
...
- The whole per-dispatch scratch directory (body file + composed page) is removed at end of dispatch. A `trap … EXIT` cannot span the multi-Bash-call + `claim-extractor` Task dispatch, so make cleanup an explicit final step: in the last Bash call, recompute the path and sweep the whole directory — `rm -rf "${TMPDIR:-/tmp}/cogni-knowledge-ingest-${SLUG}"` — not individual files, so no artifact leaks. A leftover `$WORK_DIR` is tolerable (the next same-`SLUG` dispatch's step-6 `rm -rf` clears it).
