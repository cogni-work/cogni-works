...
tests/               Plugin test suites (CI-discovered by scripts/run-plugin-tests.py)
  ...
  test-excalidraw-canvas-lock.sh  Pins the atomic start claim in hooks/ensure-excalidraw-canvas.sh, whose byte-identical twin ships in cogni-portfolio under the same unqualified `mcp__excalidraw__*` PreToolUse matcher — so a machine with both plugins installed dispatches two copies in parallel on every tool call. The failure it guards is silent: without the claim both invocations fall through the port probe, both spawn a server, the loser dies on EADDRINUSE, and canvas.pid is left naming a dead process while every exit status stays 0. Races two real invocations against a stubbed cold canvas (PATH stubs for nc/node/curl/open/xdg-open under mktemp -d — no socket bound, no network) and asserts exactly one spawn, a live canvas.pid, a loser that waits on the port and exits 0, a swept stale claim, byte-identity of both the hook and hooks.json pairs, and — the arm no behavioural case can reach — that the release is trapped for INT/TERM and not EXIT alone, since the harness kills the hook at the 15s timeout its own hooks.json declares. The stale sweep is additionally raced against a stubbed `stat` whose flag semantics diverge from the BSD form the hook was first written against (the GNU case, where `-f` selects `--file-system` and so succeeds without reporting an mtime rather than failing over): the hook must still compute a sane age and exit 0, and the chain must try the GNU form first. That ordering is pinned twice over. Behaviourally, by a case running the hook under a flag-aware `stat` — one whose `-f` succeeds without reporting an mtime, as GNU's really does — where a BSD-first chain floors that unusable reading to 0, reads a LIVE peer's claim as ancient and carries it aside instead of leaving it alone; nothing in that case reads the hook's source, so renaming the chain cannot quietly make it green. And textually, over every `stat` call the hook contains rather than the first one found, under a count floor that cannot silently shrink — which is what covers the sweep's own second chain, whose revert leaves every behavioural case green, and what lets a macOS run catch a Linux-only defect in the portfolio copy no behavioural case here can run. A further case covers the intersection the others individually miss — two invocations contending for the SAME aged claim, which is where a sweep that retires a stale claim non-atomically resurrects the double spawn on its own recovery path. It is deterministic rather than a repeated race: the racers are released together off a gate file and then staggered by a stubbed `date` (the hook's only call, sitting between reading the claim's mtime and acting on it), so the breaking interleaving is constructed instead of waited for — a case that catches the defect at some rate below 1 reports green most of the time and guards nothing. All nine documented mutation recipes were replayed against the shared harness and returned guard_verified.
