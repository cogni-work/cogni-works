...
4a. **Provider units** — renders from either of two sources (scan-output takes precedence when present). Fully absent when neither source yields a provider unit.
   - **Source 1 — `scan-output` (diagnostic)**: shown when `research/.metadata/scan-output.json` v1.2.0+ exists with a non-empty `provider_units` array (portfolio was built via `portfolio-scan`). Section header reads "Provider units (scan diagnostic)" plus a **consolidation-mode badge** sourced from `scan-output.json.consolidation_mode` (v1.3.0+): green `consolidated`, yellow `shadow review`, neutral `research only`. When absent (pre-v1.3.0 scans) the mode defaults to `consolidate`. Footer shows scan date, `scan-output.json v1.2.0+`, and `mode: <consolidation_mode>`. When the mode is `shadow` and `research/scan-candidates/{company_slug}/` contains candidate JSONs, a **Shadow candidates** block renders below the unit cards — total count plus a compact per-company slug list so reviewers can locate candidate files on disk before promoting them into `features/`. Diagnostic only — authoritative feature-to-unit mapping lives in each feature's `source_lineage`.
