...
- "These recommendations can be imported into your {industry}/{subsector} catalog.
  Run `/trends-catalog import` to review and approve the additions."

**If no catalog exists:**
- "There's no industry catalog for {industry}/{subsector} yet. You can create one
  with `/trends-catalog init` and then import these recommendations to build a reusable
...
`/trends-catalog import` operation. The catalog skill handles the actual write-back with user
