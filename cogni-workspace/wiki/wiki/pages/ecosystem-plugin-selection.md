...
**cogni-workspace (`narrative`)** — Story-arc-driven transformation. Seven narrative frameworks including a TIPS-native trend panorama, executive synthesis, and citation bridging. Takes structured content in, hands a narrative to the `story-to-*` brief producers or to `copywriter`.

**cogni-workspace (`copywriter`)** — Polishing with messaging frameworks (BLUF, Pyramid, SCQA, STAR, PSB, FAB), stakeholder review via parallel persona Q&A, and readability optimization. Also handles German documents in the Wolf Schneider register.
