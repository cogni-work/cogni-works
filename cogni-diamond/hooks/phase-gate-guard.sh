...
# Find diamond-project.json in current workspace
PROJECT_JSON=$(find . -maxdepth 3 -name "diamond-project.json" -path "*/cogni-diamond/*" 2>/dev/null | head -1)
...
with open(f"{project_dir}/diamond-project.json") as f:
    project = json.load(f)
