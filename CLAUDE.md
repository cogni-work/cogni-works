...
- Plugin and repo-root `tests/*.sh` are CI-enforced — `scripts/run-plugin-tests.py` (CI: "Plugin test suites" in `.github/workflows/lint.yml`) discovers `tests/*.sh` and `cogni-*/tests/*.sh` and fails the build on any non-zero exit. Run the full sweep locally with `python3 scripts/run-plugin-tests.py`, or one plugin's with `--filter <plugin>`. A suite only needs to exit non-zero on failure, run as `bash <path>` with no arguments, and touch no network — the runner reads exit status and nothing else
