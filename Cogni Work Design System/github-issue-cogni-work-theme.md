# Deepen the `cogni-work` theme: add type scale, spacing/radii/shadow/motion tokens, voice guidelines, font embed snippet

## Summary

The current `cogni-workspace/themes/cogni-work/theme.md` is a faithful brand spec for the **palette**, **font families**, and **design philosophy**, but it's missing the deeper layers downstream skills need to produce on-brand output without reinventing them. This issue proposes replacing the current `theme.md` with a richer version that mirrors what `themes/deutsche-telekom/theme.md` and `themes/telekom-tsc/theme.md` already do (type scale, font embed snippet, more granular design principles), plus several **new sections** that all themes will eventually need (spacing, radii, shadows, motion, voice/copy).

The full proposed replacement is at the bottom of this issue.

---

## Why now

While building the cogni-work design system in detail (tokens, UI kits for the website + plugin surfaces, 7-slide deck template, preview cards, voice guidelines, asset library), it became clear that:

1. **`theme.md` is the only file `discover-themes.py` can read.** Everything outside `theme.md` (showcase JSX, asset folders, component libraries) is invisible to the picker and unreadable by skills that resolve themes by path.
2. **The current `cogni-work/theme.md` underspecifies the brand.** It lists colors and font families but no type scale, no spacing/radii/shadow/motion scales, no font CDN snippet, no voice rules. The information exists in `cogni-work-theme-showcase.jsx` but downstream skills can't extract it from JSX reliably.
3. **`telekom-tsc` and `deutsche-telekom` are richer than `cogni-work`** — they include type scales and (in `deutsche-telekom`) the HTML font embed snippet. `cogni-work`, the *flagship* theme cited as the canonical reference in `manage-themes/SKILL.md`, should at minimum match their depth.
4. **Voice/copy guidelines belong in the theme.** Voice is part of brand identity. Skills like `cogni-copywriting`, `cogni-narrative`, and `cogni-sales` would benefit from reading voice rules from the same file they read colors from.

---

## What changes

**File:** `cogni-workspace/themes/cogni-work/theme.md`

**Action:** Replace contents (preserve current file as `theme.v1.md` for reference, or rely on git history).

**New sections added vs. current `theme.md`:**

| Section | Currently in `cogni-work/theme.md`? | Currently in `telekom-tsc/theme.md`? | Currently in `deutsche-telekom/theme.md`? |
|---|---|---|---|
| Color Palette | ✅ | ✅ | ✅ |
| Status Colors | ✅ | ✅ | ✅ |
| Typography (font families) | ✅ | ✅ | ✅ |
| **Type Scale** (px, line-height, weight per role) | ❌ | ✅ | ✅ |
| **Web Embedding (HTML font snippet)** | ❌ | ❌ | ✅ |
| Design Principles (8–10 actionable rules) | ✅ (7 short) | ✅ (7) | ✅ (8) |
...
The four "❌ across the board" rows (spacing, radii, shadow, motion, voice) are net-new conventions. They're proposed for `cogni-work` first because (a) we have the data, (b) it's the flagship theme, and (c) it sets a precedent the other two themes can adopt incrementally.

---

## Impact on existing skills

### `pick-theme` / `discover-themes.py`
**No code changes required.** The script only parses H1, the description line, primary/accent/background hex codes, and headers font — all preserved at the same heading levels. The deeper `theme.md` is fully backwards-compatible with the discovery layer.

### `manage-themes`
**Template update recommended.** `cogni-workspace/themes/_template/theme.md` should be updated to include the new sections (spacing, radii, shadows, motion, voice, type scale, web embedding) so future themes created via Operations 3–6 inherit the richer structure. Operation #7 (Generate Theme Showcase) already references `cogni-work-theme-showcase.jsx` as the canonical example — no change needed there.

### Downstream consumers (`cogni-visual`, `cogni-portfolio`, `document-skills:pptx`, etc.)
**No code changes required.** Skills that already read `theme.md` will see additional sections they can ignore or use. Skills that want to lift the new tokens (e.g. for HTML/CSS outputs) can do so via simple regex parsing of the markdown — the format is consistent with existing sections.

### `cogni-work-theme-showcase.jsx`
**No change.** The showcase remains the visual reference; the deeper `theme.md` is the *machine-readable* counterpart.

---

## Open questions

1. **Should the new sections (spacing, radii, shadows, motion, voice) be backported to `_template/theme.md` and the other two themes (`deutsche-telekom`, `telekom-tsc`) in this issue, or as follow-ups?** Recommendation: backport `_template/theme.md` in this PR; leave the other two themes for separate issues (they'd need brand-specific values).
2. **Voice section format — keep prose-heavy or formalize?** The proposal below uses bullet rules with concrete examples. Skills like `cogni-copywriting` could parse this; humans can read it.
3. **Should the deep design system project (UI kits, slide templates, asset library) be referenced by URL in the Source section?** The current proposal does this. Alternative: copy selected assets into `themes/cogni-work/assets/` for full self-containment. That'd require a `discover-themes.py` change to surface non-`theme.md` files, so leaving as a URL pointer for v1.

---

## Proposed `cogni-workspace/themes/cogni-work/theme.md`
...
- [ ] Replace `cogni-workspace/themes/cogni-work/theme.md` with the proposed version above.
- [ ] Verify `python3 cogni-workspace/skills/pick-theme/scripts/discover-themes.py` still discovers `cogni-work` with correct name, description, primary, accent, background, and font fields (no parser regressions).
- [ ] Verify `cogni-work-theme-showcase.jsx` still matches the theme.md tokens — any drift between the two should be reconciled in favor of the showcase (it's the production reference).
- [ ] (Recommended) Update `cogni-workspace/themes/_template/theme.md` to include the new sections (Type Scale, Web Embedding, Spacing Scale, Radii, Shadow Scale, Motion, Voice & Copy Guidelines) so future themes inherit the richer structure.
- [ ] (Optional follow-up) Backport spacing/radii/shadow/motion/voice sections to `deutsche-telekom` and `telekom-tsc` themes — separate issues.

---

## Notes for the implementer

- The proposal preserves the H1, the first-line description, and the existing `**Primary**`, `**Accent**`, `**Background**`, `**Headers**` patterns that `discover-themes.py` regex-matches on. Discovery is fully backwards-compatible.
- The four code blocks above (font CDN, spacing/radii/shadow values) use single backticks-as-fences in this issue body to avoid markdown nesting; in the actual `theme.md`, restore them to triple-backtick fenced code blocks.
- All hex values, spacing tokens, radii, shadows, motion durations, and type scale values come directly from the production `colors_and_type.css` in the cogni-work design system project.
