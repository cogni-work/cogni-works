#!/usr/bin/env bash
# Reads engagement state from diamond-project.json and plugin project states.
...
if [ ! -f "${PROJECT_DIR}/diamond-project.json" ]; then
  echo "{\"success\": false, \"error\": \"No diamond-project.json found in ${PROJECT_DIR}\"}"
  exit 0
fi
...
# Read diamond-project.json
with open(project_dir / "diamond-project.json") as f:
    project = json.load(f)
...
if current_status == "pending":
    ...
    if idx < len(phase_order) - 1:
        ...
        next_actions.append({"action": "Generate deliverable package", "skill": "diamond-export"})
