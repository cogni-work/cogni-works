...
5. **Bootstrap delivery blueprints and shared solution eligibility.** Before discussing blueprint adaptation, ensure every product in the batch actually has a delivery blueprint. Products without blueprints get solutions designed from scratch with no structural consistency — different phases, pricing ratios, and assumptions across markets for the same product. This step fixes that by proposing blueprints proactively.
   ...
   **Commercial-structure consolidation gate — check `commercial_model_shared` first.** Read `commercial_model_status.matrix` from `project-status.sh` (one entry per product×market). When `commercial_model_shared` is `true` — a shared/fixed revenue model (`revenue_model` ∈ {subscription, hybrid, partnership}) — **recommend consolidation**: a single shared reference cut via `shared_solution`, or a small package ladder (`packages`). Do this *before* the blueprint check below; the gate is **not** conditional on a `delivery_blueprint` already existing — a product with a decided shared model but an existing blueprint must still consolidate rather than tier one solution per proposition. If the user accepts, set `shared_solution: true` on the product so step 7 handles it as a shared solution and **skip the 1:1 per-proposition fan-out** for it. If the user declines consolidation, that product carries no `shared_solution` flag and falls back to the standard per-proposition fan-out in step 8 — it is never left without solutions. Products with a `project`-style revenue model are not gated here and proceed to step 8 as usual. (The user-facing recommendation text is emitted by `project-status.sh` in the project's output language.)
