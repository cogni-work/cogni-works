...
- Every repo-relative path a **live attribution surface** claims must resolve in the tracked index — `scripts/check-attribution-path-integrity.py` (CI: "Attribution path guard (every path a live attribution surface claims resolves)" in `.github/workflows/lint.yml`) scans the class of files this repo maintains as a *current* record of third-party material — a surface **name** class crossed with a bounded, non-recursive **location** class — so the root `LICENSE` and every per-plugin `LICENSE` are scanned and clean by CONTENT rather than skipped by NAME, and a `See:` line added to one tomorrow is checked with nothing to edit. A claim is carried by an assertion field, a code span or a markdown link target, and only if it contains `/` and no `://`, so a URL, a bare filename and an upstream slug carried by a provenance field are excluded by shape and by field role, never by matching their text. The frozen attestation under `docs/relicensing/` is out of scope by the class rule in `docs/contributing/plugin-absorption-slicing.md` — a surface declaring its own "as of" date does not describe the tree now — and the exclusion is structural, since the location class never descends into a documentation tree. Resolution reads `git ls-files`, not `os.path.exists`, because CI is case-sensitive while a contributor's macOS clone usually is not; it borrows `check-breadcrumbs.py`'s tracked-content lookup and explicitly not its baseline ratchet. Zero discovered surfaces is a failure, not a clean zero. Do not re-derive the rest here — the guard's module docstring owns the scope decision, the rejected alternatives, the exact name and location classes, and the recall floor (including its deliberate discovery-depth bound); a second copy is a second thing to drift. Hard clean zero — no baseline, allowlist or skip marker. Stdlib-only; suite at `tests/test_check_attribution_path_integrity.sh`
