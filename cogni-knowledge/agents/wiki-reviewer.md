...
- **Inline citations**: the superscript-wrapped numeric link the composer emits, `<sup>[N](url)</sup>` — regex approximately `<sup>\[\d+\]\([^)]+\)</sup>`. Also count the URL-less fallback `<sup>[N]</sup>` (regex `<sup>\[\d+\]</sup>`) for sources with an empty URL field.
  **Under the author-date citation family also count the author-date marker** — regex approximately `\(\[[^\[\]]+\]\((?:https?|file)://[^)]+\)\)`, the `([Author, Year](url))` / `([Author](url))` / `([Author Year](url))` shapes the composer emits for `apa` / `mla` / `harvard`. A fully-cited author-date section carries no `[N]` at all, so a numbered-only scan would score it as a citation-density deficit purely because of the marker shape. The URL-less `<sup>[N]</sup>` fallback above stays counted in this family too — a synthesis, distilled or question-node citation has no destination to link and keeps the plain superscript. Two properties of that pattern are load-bearing and must not be relaxed: it is **scheme-anchored**, so ordinary parenthesized prose links like `(see [the annex](#section-3))` are not counted as citations; and its label class **excludes `[` and `]`**, so it can never match the `[[N]]` anti-pattern the next paragraph flags.
