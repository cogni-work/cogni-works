...
| Discovered offering | **Feature** (`features/{slug}.json`) | **Solution-seed** entry in `research/.staging/scan-solutions-draft.json` — one per delivery stack per category | In `consolidate` / `shadow`, each offering is its own feature (per-provider differentiation). In `category-aggregation`, offerings are rolled up as delivery-stack variants (OTC / AWS / GCP / on-prem / …) under the category-grained feature; `solutions/` seeds per-stack solution entities from the staging artifact. |
