...
After the script creates directories, write `portfolio.json` in the project root with the confirmed company context, including the `language` field. Follow the schema in `$CLAUDE_PLUGIN_ROOT/references/data-model.md`.
...
For complete entity schemas and naming conventions, consult `$CLAUDE_PLUGIN_ROOT/references/data-model.md`.
