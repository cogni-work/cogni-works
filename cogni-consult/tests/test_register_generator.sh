...
mkdir -p "$ENG"
OUT=$(python3 "$SCRIPT" "$ENG")
assert_success "missing-registry-envelope" true "$OUT"
grep -q 'No assumptions registered yet.' "$ENG/assumptions.md" \
...
grep -q 'Do not clobber.' "$ENG/assumptions.md" \
  && pass "overwrite-guard-leaves-file-intact" || fail "overwrite-guard-leaves-file-intact" "$(cat "$ENG/assumptions.md")"
