...
- Every `FAIL: <id>` arm in a discovered suite must have a reachable same-id green (`PASS:`/`ok:`) arm — `scripts/check-case-id-pairing.py` (CI: "Case-id pairing guard (every FAIL arm has a same-id green twin)" in `.github/workflows/lint.yml`) enforces `cogni-knowledge/tests/README.md` rule 7 mechanically over `tests/*.sh` and `*/tests/*.sh`. A case whose green arm can never print emits nothing on a green run, so `mutation-check.sh --case <id>` reports `case_not_found` on a genuinely red run and the guard it was meant to check proves nothing. Detection resolves **one level of helper indirection** — a helper carrying both labels is a paired emitter whose call sites are self-pairing, while a single-arm helper makes its call sites emissions with the id read from `$1` — because a same-line scan is blind to the dominant emission shape here and would ship green and dead. The **exemption keys on scope, not on abort**: a fail-only arm is exempt only when it aborts AND sits at script scope, so the aborting-inside-a-loop shape stays a finding. That is deliberately weaker than rule 7's own text, which says aborting alone does not earn the exemption; the guard's docstring owns the reasoning, the stated recall floor (out-of-glob sourced helpers, whole-variable ids, heredoc bodies, labels not in command position) and what a future narrowing would cost. Do not re-derive any of it here. Hard clean zero — no baseline, allowlist or skip marker; suite at `tests/test_check_case_id_pairing.sh`
