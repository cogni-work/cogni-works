...
if [ ! -f "$SCRIPT" ]; then
  red "FAIL: ingest-post-01 scripts/knowledge-ingest-postprocess.py not found"
  exit 1
fi
assert_grep 'argparse' "$SCRIPT" "ingest-post-02 stdlib argparse (no pip deps)"
assert_grep '--project-path' "$SCRIPT" "ingest-post-03 --project-path flag"
assert_grep '--wiki-root' "$SCRIPT" "ingest-post-04 --wiki-root flag"
assert_grep '--wiki-scripts-dir' "$SCRIPT" "ingest-post-05 --wiki-scripts-dir flag"
assert_grep '--new-slugs' "$SCRIPT" "ingest-post-06 --new-slugs structured input (no shell interpolation)"
assert_grep '--binding' "$SCRIPT" "ingest-post-07 --binding flag (question-store emit + upsert-themes)"
assert_grep '--knowledge-root' "$SCRIPT" "ingest-post-08 --knowledge-root flag (upsert-themes)"
assert_grep 'from _knowledge_lib import sanitize_summary' "$SCRIPT" "ingest-post-09 sanitizes summaries via _knowledge_lib (no env-var python3 -c)"
assert_grep 'backlink_audit.py' "$SCRIPT" "ingest-post-10 shells out to backlink_audit.py (unchanged vendored helper)"
assert_grep 'wiki_index_update.py' "$SCRIPT" "ingest-post-11 shells out to wiki_index_update.py (unchanged vendored helper)"
assert_grep 'config_bump.py' "$SCRIPT" "ingest-post-12 shells out to config_bump.py (unchanged vendored helper)"
assert_grep 'question-store.py' "$SCRIPT" "ingest-post-13 shells out to question-store.py emit"
assert_grep 'sub_index.py' "$SCRIPT" "ingest-post-14 shells out to sub_index.py render"
assert_grep 'knowledge-binding.py' "$SCRIPT" "ingest-post-15 shells out to knowledge-binding.py upsert-themes"
assert_grep 'entries_count' "$SCRIPT" "ingest-post-16 bumps entries_count"
assert_grep 'inserted' "$SCRIPT" "ingest-post-17 counts n_new/n_new_q on action == inserted (drift fix in one place)"
assert_grep 'max-summary' "$SCRIPT" "ingest-post-18 passes --max-summary clamp (byte-identical arg)"
assert_grep 'create-missing-heading' "$SCRIPT" "ingest-post-19 R1 reverse link uses --create-missing-heading"
assert_grep 'question-manifest.json' "$SCRIPT" "ingest-post-20 persists question-manifest.json handoff"
...
assert_not_grep '_wiki_lock(' "$SCRIPT" "ingest-post-21 does NOT acquire _wiki_lock itself (no deadlock shape)"
assert_not_grep 'import requests' "$SCRIPT" "ingest-post-22 stdlib only (no requests)"

# === Contract: knowledge-ingest SKILL.md =================================
if [ ! -f "$INGEST" ]; then
  red "FAIL: ingest-post-23 skills/knowledge-ingest/SKILL.md not found"
  exit 1
fi
assert_grep 'knowledge-ingest-postprocess.py' "$INGEST" "ingest-post-24 Step 4 calls the orchestrator script"
assert_grep '--new-slugs' "$INGEST" "ingest-post-25 passes the this-run slug list as structured input"
# The LLM backlink audit + curation middle step STAYS in SKILL.md.
assert_grep 'backlink_audit.py' "$INGEST" "ingest-post-26 keeps the per-slug backlink audit (LLM-curated middle step)"
assert_grep '.backlink-plan' "$INGEST" "ingest-post-27 writes per-slug curated backlink plan files the orchestrator applies"
# Step 4.6 contradiction tripwire stays in SKILL.md (agent dispatch).
assert_grep 'source-contradictor' "$INGEST" "ingest-post-28 keeps the Step 4.6 contradiction tripwire (agent dispatch)"
...
if printf '%s' "$ERR_OUT" | python3 -c "import json,sys;d=json.load(sys.stdin);sys.exit(0 if (d['success'] is False and 'ingest-manifest' in (d['error'] or '')) else 1)" 2>/dev/null; then
  green "PASS: ingest-post-29 structural error envelope on missing ingest-manifest.json"
else
  red "FAIL: ingest-post-29 expected success=false + ingest-manifest error; got: $ERR_OUT"
  errors=$((errors + 1))
fi
...
if printf '%s' "$NOOP_OUT" | python3 -c "import json,sys;d=json.load(sys.stdin)['data'];sys.exit(0 if (d['n_new']==0 and d['sources_subindex']=='unchanged' and d['questions_subindex']=='unchanged') else 1)" 2>/dev/null; then
  green "PASS: ingest-post-30 no-op path (n_new=0, sub-indexes unchanged)"
else
  red "FAIL: ingest-post-30 no-op path; got: $NOOP_OUT"
  errors=$((errors + 1))
fi
...
if printf '%s' "$HAPPY_OUT" | python3 -c "import json,sys;d=json.load(sys.stdin)['data'];sys.exit(0 if (d['n_new']==1 and d['n_new_q']==1 and d['reverse_links_applied']==1 and d['sources_subindex']=='rendered' and d['questions_subindex']=='rendered' and d['question_manifest']=='written') else 1)" 2>/dev/null; then
  green "PASS: ingest-post-31 happy path (n_new=1, question emitted+reverse-linked, sub-indexes rendered)"
else
  red "FAIL: ingest-post-31 happy path; got: $HAPPY_OUT"
  errors=$((errors + 1))
fi
# sanitize_summary applied: the dagger must NOT survive into the index one-liner.
if grep -q 'X†topic' "$WIKI/wiki/index.md"; then
  red "FAIL: ingest-post-32 summary dagger leaked into wiki/index.md (sanitize_summary not applied)"
  errors=$((errors + 1))
else
  green "PASS: ingest-post-32 summary sanitized before reaching wiki/index.md"
fi
...
if [ "$(python3 -c "import json;print(json.load(open('$WIKI/.cogni-wiki/config.json'))['entries_count'])")" = "2" ]; then
  green "PASS: ingest-post-33 entries_count bumped once per newly-inserted row (=2)"
else
  red "FAIL: ingest-post-33 entries_count != 2"
  errors=$((errors + 1))
fi
# question-manifest handoff written
if [ -f "$PROJ/.metadata/question-manifest.json" ]; then
  green "PASS: ingest-post-34 question-manifest.json handoff written"
else
  red "FAIL: ingest-post-34 question-manifest.json not written"
  errors=$((errors + 1))
fi
