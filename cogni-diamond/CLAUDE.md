# cogni-diamond Development Guide

## Identity

cogni-diamond is a Double Diamond consulting orchestrator for the cogni-works ecosystem. It manages engagement state and steers the consulting process through four phases (Discover, Define, Develop, Deliver), dispatching to existing plugins rather than producing content itself.

## Architecture

```
diamond-setup skill       → Vision framing, engagement scaffolding
diamond-discover skill    → D1 diverge: cogni-gpt-researcher, cogni-tips, cogni-portfolio
diamond-define skill      → D1 converge: cogni-claims, guided synthesis
diamond-develop skill     → D2 diverge: cogni-tips value-modeler, cogni-portfolio propositions
diamond-deliver skill     → D2 converge: cogni-claims, business case, roadmap
diamond-resume skill      → Multi-session re-entry, status dashboard
diamond-export skill      → Deliverable package generation via cogni-visual + document-skills
...
Each engagement lives in `cogni-diamond/{slug}/` with:
- `diamond-project.json` — engagement config, vision, phase state, plugin refs
...
| `engagement-status.sh` | Read diamond-project.json + plugin states → JSON |
...
- Language field in diamond-project.json controls communication language (technical terms stay English)
