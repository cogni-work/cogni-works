...
4a. **Provider units (scan diagnostic)** — (shown only when `research/.metadata/scan-output.json` v1.2.0+ exists with a non-empty `provider_units` array, i.e., the portfolio was built via `portfolio-scan`) Cards per included provider unit with feature count, domain, and type chip. Footer line shows included/total, cumulative feature count, domains analyzed, scan date, and scan-output schema version. Diagnostic only — authoritative feature-to-unit mapping lives in each feature's `source_lineage`. Fully absent for portfolios built without `portfolio-scan` (no file = no section, no nav link).
