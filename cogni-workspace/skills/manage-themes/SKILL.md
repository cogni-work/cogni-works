...
- Read `data.unclassified` on **every** run, before reporting anything. It lists the palette roles that parsed as colours but matched no role name above, so they formed no pair. A non-empty `unclassified` means the audit is partial no matter how large `data.evaluated` is — remap those keys onto the vocabulary and re-run, or grade them explicitly with `--pair` / `--large-pair`. Never present a contrast result as complete while it is non-empty.
