...
If `commercial_model_status` is present and `any_shared` is true, add a commercial-structure line before recommending solutions: a shared/fixed commercial structure was detected for one or more products — via a shared `revenue_model` (∈ {subscription, hybrid, partnership}), a fixed/catalog `commercial_model` (∈ {catalog, subscription, usage, partnership}), an off-enum `revenue_model` with a documented shared disposition (e.g. `transaction-fee`, `product_and_license`), or a set of at least three propositions whose declared `commercial_model` values collapse to exactly one distinct value — so the recommended next action is to **consolidate** — a shared reference cut (`shared_solution`) or a small package ladder (`packages`/Paketleiter) — rather than generating one tiered solution per proposition. The consolidation `next_action` emitted by `project-status.sh` already carries this wording; surface it instead of an "N solutions per proposition" suggestion.
