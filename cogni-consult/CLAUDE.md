...
- **Language reaches subagents only through the hook** — a subagent's system prompt is its own body plus a notes block and the environment info: no settings `language` key, no `CLAUDE.md`, no active output style. `hooks/hooks.json` registers a `SubagentStart` hook matched on the four `consult-*` agent types; `hooks/on-subagent-start.sh` resolves the workspace default language and emits it together with `references/subagent-output-contract.md`, read verbatim. Because a hook cannot see the user's message, rung 2 of `references/interaction-language.md` (message-detection override) travels as the `interaction_language` dispatch input, which wins over the hook's default. Keep the doctrine in the reference, not in the hook script and not in the four agent bodies — the script is a delivery mechanism, and the agent bodies hold only the input declaration. `references/user-facing-output.md` is the other half: one contract, two delivery paths (that file loads in the main loop, this one is injected into agents), so a rule changed in either is checked against the other
  ...
  **`SubagentStart` is a supported event, and the matcher must accept the qualified name.** The event is real — Claude Code added it in 2.0.43 and has restricted it to command-type hooks since 2.1.142, which is what `hooks/hooks.json` declares. It was confirmed empirically on build 2.1.220 by dispatching a matched agent and having it report the context it was given. The matcher accepts both the bare `consult-*` name and the plugin-qualified `cogni-consult:consult-*` form, because a plugin-supplied agent is dispatched under the qualified name: an earlier matcher anchored on bare names alone silently never fired, so all four agents ran without the language block or the output contract while `hooks/on-subagent-start.sh` itself was correct the whole time. That failure is silent by construction — the script always exits 0 and a matcher that never matches produces no error anywhere — so `tests/test_subagent_start_hook.sh` pins both name forms. Reviewer note: `plugin-dev`'s `validate-hook-schema.sh` carries an event whitelist that predates this event and prints `Unknown event type: SubagentStart` while exiting 0; that is a gap in the validator, not evidence the event is unsupported. The `hooks` keyword advertised in `.claude-plugin/plugin.json` and mirrored in `marketplace.json` rests on this hook, so a matcher change is a marketplace-visible capability change
