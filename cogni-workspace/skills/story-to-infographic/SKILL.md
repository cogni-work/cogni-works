...
**Citation handling.** A narrative generated by the `narrative` skill ends with a bold `**Sources**` paragraph after its fourth `##`; element 4 ends at that line and the block is never panel content — use it only for the footer's source attribution. When the source contains inline citations (e.g., `[Source: Publisher](URL)`,
